			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					if ((line.split("■")[1].equals(selectedPlace.getName())
						...
						int lineDurationTime = Integer
								.parseInt(line.split("■")[5].substring(0, line.split("■")[5].indexOf("(")));
						int lineVisitCount = Integer.parseInt(

								line.split("■")[5].substring(line.split("■")[5].indexOf("(") + 1,
			...
			try {
				while ((line = reader.readLine()) != null) {
					...
					if (line.contains(selectedPlace.getName()) && num >= 5) {
						System.out.printf("\t\t\t%s님의 리뷰\t\t 작성일:%s\t 별점 : %.1f\n", temp[1], temp[3],
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (line.contains(selectedPlace.getName())) {
					System.out.printf("\t\t\t%s님의 리뷰\t\t 작성일:%s\t 별점 : %s\n", temp[1], temp[3], temp[6]);
					System.out.println("\t\t\t" + temp[2]);
					System.out.println(
							"\t\t\t============================================================================");
					i++;
